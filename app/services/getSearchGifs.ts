...
export const getSearchGifs = async ({ querySearch }: { querySearch: string }): Promise<Daum[]> => {
  const res = await fetch(`${API_URL}/search?${API_KEY}&q=${querySearch}&limit=25&offset=0&rating=g&lang=en`)
  const data: SearchGifs = await res.json()
  return data.data
}
